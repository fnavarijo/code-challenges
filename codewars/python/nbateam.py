...
class TeamStatistics:
    ...
    def __str__(self):
        return self.teamName + ' ' + self.score;

def parseMatches(match):
    teamsInfo = re.search('([\D]+)([0-9]*)([\D]+)([0-9]*)', match);
    firstTeam = TeamStatistics(teamsInfo.group(1), teamsInfo.group(2));
    secondTeam = TeamStatistics(teamsInfo.group(3), teamsInfo.group(4));
    print('%s - %s' % (firstTeam, secondTeam))
    return len(match);

def nba_cup(result_sheet, to_find):
    matches = result_sheet.split(',');
    parsedMatches = list(map(parseMatches, matches));
    print(parsedMatches);
